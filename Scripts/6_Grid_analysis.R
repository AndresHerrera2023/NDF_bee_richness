...
spdf75_zero <- length(spdf75[spdf75$Richness == 0]) #389
spdf75_pct_zero <- (spdf75_zero / spdf75_all) * 100 #47.55 are zero.
...
spdf100_zero <- length(spdf100[spdf100$Richness == 0]) #208
spdf100_pct_zero <- (spdf100_zero / spdf100_all) * 100 #39.17 are zero.
...
gndf50_10_80_pct <- (sum(gndf50_counts[2:8]) / sum(gndf50_counts)) * 100 # 27.21%
gndf50_10_80_pct

# Grids > 80 genera
gndf50_gt80_pct <- (sum(gndf50_counts[9:10]) / sum(gndf50_counts)) * 100 # 0.48%
gndf50_gt80_pct
...
gndf75_gt80_pct <- (sum(gndf75_counts[9:11]) / sum(gndf75_counts)) * 100 # 0.93%
gndf75_gt80_pct 
...
# Grids with 10-80 genera
gndf100_10_80_pct <- (sum(gndf100_counts[2:8]) / sum(gndf100_counts)) * 100 # 37.15%
gndf100_10_80_pct

# Grids with > 80 genera
gndf100_gt80_pct <- (sum(gndf100_counts[9:11]) / sum(gndf100_counts)) * 100 # 1.54%
gndf100_gt80_pct 
...
sptw50_zero <- length(sptw50[sptw50$Richness == 0]) # 947
sptw50_zero_pct <- (sptw50_zero / sptw50_all) * 100 # 64.20%

# Grids with 1-20 species
sptw50_nonzero <- sptw50[sptw50$Richness != 0,]
sptw50_hist <- hist(sptw50_nonzero$Richness, breaks = 10)
sptw50_counts <- sptw50_hist[[2]]
sptw50_1_20_pct <- (sptw50_counts[1] / sum(sptw50_counts)) * 100 # 83.71%
sptw50_1_20_pct

# Grids with 20-100 species
sptw50_20_100_pct <- (sum(sptw50_counts[2:5]) / sum(sptw50_counts)) * 100 # 14.39%
sptw50_20_100_pct

# Grids with >100 species
sptw50_gt100_pct <- (sum(sptw50_counts[6:11]) / sum(sptw50_counts)) * 100 # 1.89%
sptw50_gt100_pct
...
sptw75_zero <- length(sptw75[sptw75$Richness == 0]) # 370
sptw75_zero_pct <- (sptw75_zero / sptw75_all) * 100 # 50.54%

# Grids with 1-20 species
sptw75_nonzero <- sptw75[sptw75$Richness != 0,]
sptw75_hist <- hist(sptw75_nonzero$Richness, breaks = 10)
sptw75_counts <- sptw75_hist[[2]]
sptw75_1_20_pct <- (sptw75_counts[1] / sum(sptw75_counts)) * 100 # 76.79%
sptw75_1_20_pct 

# Grids with 20-100 species
sptw75_20_100_pct <- (sum(sptw75_counts[2:5]) / sum(sptw75_counts)) * 100 # 19.33%
sptw75_20_100_pct 

# Grids with >100 species
sptw75_gt100_pct <- (sum(sptw75_counts[6:12]) / sum(sptw75_counts)) * 100 # 3.86%
sptw75_gt100_pct
...
sptw100_zero <- length(sptw100[sptw100$Richness == 0]) # 180
sptw100_zero_pct <- (sptw100_zero / sptw100_all) * 100 # 38.96%
...
sptw100_1_20_pct <- (sptw100_counts[1] / sum(sptw100_counts)) * 100 # 72.34%

# Grids with 20-100 species
sptw100_20_100_pct <- (sum(sptw100_counts[2:5]) / sum(sptw100_counts)) * 100 # 22.69%

# Grids with >100 species
sptw100_gt100_pct <- (sum(sptw100_counts[6:13]) / sum(sptw100_counts)) * 100 # 4.96%
...
# Grids with 10-80 genera
gntw50_10_80_pct <- (sum(gntw50_counts[2:8]) / sum(gntw50_counts)) * 100 # 26.51%

# Grids with >80 genera
gntw50_gt80_pct <- (sum(gntw50_counts[9:10]) / sum(gntw50_counts)) * 100 # 0.37%
...
gntw75_1_10_pct <- (gntw75_counts[1] / sum(gntw75_counts)) * 100 # 69.33%

# Grids with 10-80 genera
gntw75_10_80_pct <- (sum(gntw75_counts[2:8]) / sum(gntw75_counts)) * 100 # 29.83%

# Grids with >80 genera
gntw75_gt80_pct <- (sum(gntw75_counts[9:10]) / sum(gntw75_counts)) * 100 # 0.82%
...
gntw100_1_10_pct <- (gntw100_counts[1] / sum(gntw100_counts)) * 100 # 63.47%
gntw100_1_10_pct

# Grids with 10-80 genera
gntw100_10_80_pct <- (sum(gntw100_counts[2:8]) / sum(gntw100_counts)) * 100 # 35.46%
gntw100_10_80_pct
